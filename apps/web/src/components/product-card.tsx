    ...
    <div className="group rounded-2xl border border-border bg-card overflow-hidden transition-shadow hover:shadow-md">
      <Link href={`/produit/${product.seoUrl}`} className="relative block aspect-square bg-brand-primary-soft">
        <div className="absolute left-2 top-2 flex flex-col gap-1">
          {hasPromo && <Badge variant="cta">PROMO</Badge>}
          {lowStock && <Badge variant="warning">DERNIÈRES PIÈCES</Badge>}
          {outOfStock && <Badge variant="outline">ÉPUISÉ</Badge>}
        </div>
        <button
          className="absolute right-2 top-2 flex size-8 items-center justify-center rounded-full bg-card/90 text-muted-foreground shadow-sm hover:text-brand-cta"
